...
import (
	...
	"errors"
	"fmt"
	specs "quickstart/gen/travel/agents/planner/specs"
	"time"
...
// RegisterPlannerAgent registers the generated agent components with the runtime.
func RegisterPlannerAgent(ctx context.Context, rt *agentsruntime.Runtime, cfg PlannerAgentConfig) error {
	if rt == nil {
		return errors.New("runtime is required")
	}
	agent, err := NewPlannerAgent(cfg)
	if err != nil {
		return err
	}
...
func RegisterUsedToolsets(ctx context.Context, rt *agentsruntime.Runtime, opts ...func(map[string]agentsruntime.ToolCallExecutor)) error {
	...
	{
		const toolsetID = "travel.budget_tools"
...
// WithBudgetToolsExecutor associates an executor for travel.budget_tools.
func WithBudgetToolsExecutor(exec agentsruntime.ToolCallExecutor) func(map[string]agentsruntime.ToolCallExecutor) {
	return func(m map[string]agentsruntime.ToolCallExecutor) {
		if exec == nil {
			return
		}
		m["travel.budget_tools"] = exec
	}
}
